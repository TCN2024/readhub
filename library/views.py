...
def addbook_view(request):
    ...
    if request.method=='POST':
        ...
        book = models.Book.objects.filter(isbn=request.POST.get('isbn'))
        if len(book) > 0:
            messages.add_message(request, messages.ERROR, 'Book already added!')
...
def deletebook_view(request,book_id):
    ...
    issued = models.IssuedBook.objects.filter(isbn=book.isbn)
    if len(issued) == 0:
        book.delete()
    else:
        messages.add_message(request, messages.ERROR, 'Cannot delete issued book')
...
def issuebook_view(request):
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            book = models.Book.objects.filter(isbn=request.POST.get('isbn2'))[0]
